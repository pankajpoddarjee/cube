...
class ProjectModel extends CI_Model
{
	...
	{  
        $result =  $this->db->get_where('projects', ['id' => $id])->row();
		return $result;
	}
    ...
    public function update_project_data($id,$data){
    ...
    public function is_exist_for_insert($table,$data){
        $result = $this->db->where('project_name', $data)->get($table)->result();;
		return $result;
    }
    public function is_exist_for_update($table,$id,$data){
        $result = $this->db->where('project_name', $data)->where('id !=', $id)->get($table)->result();;
		return $result;
    }

    public function get_all_jobs($project_id)
	{
        $result =  $this->db->get_where('jobs', ['project_id' => $project_id])->result();
		return $result;
	}

    public function get_one_job($id)
	{  
        $result =  $this->db->get_where('jobs', ['id' => $id])->row();
		return $result;
	}
    ...
    public function delete_job($id,$data){

        $result = $this->db->where('id', $id)->update('jobs', $data);
		return $result;
    }
    public function update_job_data($id,$data){
        $result = $this->db->where('id', $id)->update('jobs', $data);
		return $result;
    }

    public function is_exist_for_job_insert($table,$job_name,$project_id){
        $result = $this->db->where('job_name', $job_name)->where('project_id', $project_id)->get($table)->result();;
		return $result;
    }
    public function is_exist_for_job_update($table,$id,$job_name,$project_id){
        $result = $this->db->where('job_name', $job_name)->where('project_id', $project_id)->where('id !=', $id)->get($table)->result();;
		return $result;
    }

    public function get_all_properties()
	{
        $result =  $this->db->get_where('properties', ['is_active' => 1])->result();
		return $result;
	}
