...
class ProjectController extends CI_Controller {
   ...
   public function __construct() {
      ...
      $this->load->model('admin/ProjectModel', 'project');
   ...
   { 
       $data['projects'] = $this->project->get_all_project();
       //$this->load->view('admin/post_type', $data);

       $data['title'] = ucfirst('Projects'); // Capitalize the first letter
       $data['content'] = $this->load->view('admin/project',$data, true); // Load view content
   ...
   { 
       ...
       $data['title'] = ucfirst('Add Project'); // Capitalize the first letter
       $data['content'] = $this->load->view('admin/save_project',$data, true); // Load view content
       $this->load->view('admin/layout', $data);
   }
   public function editProject($id)
   {  
       $data['project'] = $this->project->get_one_project($id);

       $data['title'] = ucfirst('Edit Project'); // Capitalize the first letter
       $data['content'] = $this->load->view('admin/save_project',$data, true); // Load view content
       $this->load->view('admin/layout', $data);
   }
   ...
   { 
       $record = $this->project->get_one_project($id);
       $status = $record->is_active;
       $updateData['is_active'] = ($status==1)?0:1;
       $delete = $this->project->delete_project($id,$updateData);
       if($delete){
   ...
   {     
           $this->load->library('form_validation');
           $this->form_validation->set_rules('project_name','Project Name','trim|required|min_length[3]|max_length[16]');
           
           if ($this->form_validation->run() == FALSE)
           {

               $title = empty($this->input->post('post_id'))?'Add Project':'Edit Project';
               ...
               $this->load->view('admin/layout', $data);
                   ...
                   if(empty($this->input->post('project_id'))){
                       $is_exist = $this->project->is_exist_for_insert('projects', $projectData['project_name']);
                       if($is_exist){
                           $this->session->set_flashdata('error', 'This Project already exist');
                           $this->session->set_flashdata('project', $this->input->post()); // Store input data
                           redirect(base_url('admin/addProject'));
                       }
                       $project_id   =  $this->project->insert_project_data($projectData);
                   }else{
                       $project_id = $this->input->post('project_id');
                       $is_exist = $this->project->is_exist_for_update('projects',$project_id, $projectData['project_name']);
                       if($is_exist){
                           $this->session->set_flashdata('error', 'This Project already exist');
                           redirect(base_url('admin/editProject/'.$this->input->post('project_id')));
                       }
                       $updated   =  $this->project->update_project_data($project_id,$projectData);
                   }
                   
                   if($project_id){
                       $this->session->set_flashdata('success', 'Project saved successfully');
                   }else{
                       $this->session->set_flashdata('error', 'Project not saved');
                   } 
   ...
   { 
       ...
       $data['jobs'] = $this->project->get_all_jobs($project_id);

       $data['title'] = ucfirst('Jobs'); // Capitalize the first letter
       $data['content'] = $this->load->view('admin/jobs',$data, true); // Load view content
       $this->load->view('admin/layout', $data);
   }

   public function addJobs($project_id)
   {  
       $data['project_id'] = $project_id;
       $data['title'] = ucfirst('Add Job'); // Capitalize the first letter
       $data['content'] = $this->load->view('admin/save_Job',$data, true); // Load view content
       $this->load->view('admin/layout', $data);
   }

   public function editJobs($project_id,$job_id)
   {   //echo $project_id;
        $data['project_id'] = $project_id;
       $data['job'] = $this->project->get_one_job($job_id);

       $data['title'] = ucfirst('Edit Job'); // Capitalize the first letter
       $data['content'] = $this->load->view('admin/save_Job',$data, true); // Load view content
       $this->load->view('admin/layout', $data);
   }

   public function deleteJobs($project_id,$job_id)
   {  
       $record = $this->project->get_one_job($job_id);
       $status = $record->is_active; 
       $updateData['is_active'] = ($status==1)?0:1;
       $delete = $this->project->delete_job($job_id,$updateData);
       if($delete){
           $this->session->set_flashdata('success', 'Status updated successfully');
       }else{
           $this->session->set_flashdata('error', 'Status not changed');
       } 
       redirect(base_url('admin/project/'.$project_id.'/jobs'));
   }

   public function saveJobs($project_id)
   {     
           $this->load->library('form_validation');
           $this->form_validation->set_rules('job_name','Job Name','trim|required|min_length[3]|max_length[16]');
           
           if ($this->form_validation->run() == FALSE)
           {
                $data['project_id'] = $project_id;
               $title = empty($this->input->post('post_id'))?'Add Job':'Edit Job';
               $data['title'] = ucfirst($title); // Capitalize the first letter
               $data['content'] = $this->load->view('admin/save_job',$data, true); // Load view content
               $this->load->view('admin/layout', $data);
                   ...
                   $jobData['job_name'] = $this->input->post('job_name');
                   $jobData['is_active'] = 1;
                   

                   if(empty($this->input->post('job_id'))){
                       $is_exist = $this->project->is_exist_for_job_insert('jobs', $jobData['job_name'],$project_id);
                       if($is_exist){
                           $this->session->set_flashdata('error', 'This Job already exist');
                           $this->session->set_flashdata('project', $this->input->post()); // Store input data
                           redirect(base_url('admin/project/'.$project_id.'/addJobs'));
                       }
                       $job_id   =  $this->project->insert_job_data($jobData);
                   }else{
                       $job_id = $this->input->post('job_id');
                       $is_exist = $this->project->is_exist_for_job_update('jobs',$job_id, $jobData['job_name'],$project_id);
                       if($is_exist){
                           $this->session->set_flashdata('error', 'This Job already exist');
                           redirect(base_url('admin/project/'.$project_id.'/editJobs/'.$this->input->post('job_id')));
                       }
                       $updated   =  $this->project->update_job_data($job_id,$jobData);
                   }
                   
                   if($job_id){
                       $this->session->set_flashdata('success', 'Job saved successfully');
                   }else{
                       $this->session->set_flashdata('error', 'Job not saved');
                   } 
                   //redirect(base_url('admin/addPostType'));
   ...
   public function estimate($project_id,$job_id)
   { 

    
       $data['project_id'] = $project_id;
       $data['job_id'] = $job_id;
       $data['properties'] = $this->project->get_all_properties();

       $data['title'] = ucfirst('Estimate'); // Capitalize the first letter
       $data['content'] = $this->load->view('admin/estimate',$data, true); // Load view content
       $this->load->view('admin/layout', $data);
   }
